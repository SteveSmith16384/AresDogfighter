...
/*
 * Checking Rays work with ghost control collisions
 */
public class PhysicsTestWithRays extends SimpleApplication implements PhysicsCollisionListener, PhysicsTickListener {
	...
	public static void main(String[] args){
		PhysicsTestWithRays app = new PhysicsTestWithRays();
	...
	public void simpleInitApp() {
		...
		collShape1 = CollisionShapeFactory.createDynamicMeshShape(ship1);
	...
	public void simpleUpdate(float tpf) {
		...
		Ray r = new Ray();
		List<PhysicsRayTestResult> colls = bulletAppState.getPhysicsSpace().rayTest(this.getCamera().getLocation(), this.ship1.getWorldTranslation());
		if (colls.size() > 0) {
			Iterator<PhysicsRayTestResult> it = colls.iterator();
			while (it.hasNext()) {
				PhysicsRayTestResult res = it.next();
				if (res.getCollisionObject().getUserObject() == this.ship2) {
					System.out.println("Ray Collisions:" + res);
				}
			}
		}
